...
MAIN:		
			...
			/*** Enabling Debug Port Test features ***/
			
			MOV DPTR, #1500H
			MOV A, #00H				// Initializing the Accumulator to serve as an
									// overflow counter for the LED timing
			MOV B, A				// Using register B as the counter in the code 
									// so that debug port functionality can be served
...
LOOP:		/*** Sending out some output to the debug port through P0 ***/
			MOV A, #0AAH
			MOVX @DPTR, A
			
			NOP						// No operation while in the infinite loop
			NOP						// This is done to ensure that there is an
			NOP						// appreciable delay in the debug port outputs
			
			/*** Sending out some output to the debug port through P0 ***/
			MOV A, #055H
			MOVX @DPTR, A
...
ISR_Timer0:	
			...
			/* Debug port test:
			 * The value stored in the accumulator is 
			 * sent out to P0 so as to write to a memory location.
			 * The debug port pins that output the stored data D0-D7
			 * can demonstrate the value stored in the accumulator
			...
			*/
			...
			INC B					// Incrementing the Timer 0 ISR counts
			MOV A, B
